...
(defn create
  "Creates and optionally connects a socket if one does not exist.
   ...
   (def socket (galactica.socket/create {:galactica.socket/id ::foo })).
   ...
   For ::connect-fn ::disconnect-fn ::send-fn see galactica.socket.nodejs_net, galactica.socket.websocket implementations"
  [{:as opts
    :keys [::socket.spec/id
           ::socket.spec/send|
           ::socket.spec/recv|
...
(defmulti send
  "Send data, data is passed directly to the underlying socket.
   See for example ::send-fn implmentation of galactica.socket.nodejs_net."
