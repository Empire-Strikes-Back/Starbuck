(ns galactica.nrepl.client.impl
  (:require
   ...
   [galactica.nrepl.client.impl.async]

   [galactica.nrepl.bencode.impl :as nrepl.bencode.impl]

   [galactica.csp.op.spec :as op.spec]
   [galactica.net.socket.spec :as net.socket.spec]
   [galactica.net.socket.chan :as net.socket.chan]

   [galactica.nrepl.client.spec :as nrepl.client.spec]
   [galactica.nrepl.client.chan :as nrepl.client.chan]))
...
(defn create-proc-ops
  ...
  (let [{:keys [::nrepl.client.chan/ops|
        ...
        recv|p (galactica.nrepl.client.impl.async/pub recv|* nrepl-op-id-key (fn [_] (sliding-buffer 10)))
        nrepl-op (fn [opts]
                   (let [{:keys [::nrepl.client.spec/done-keys
                         ...
                         release #(do
                                    ...
                                    (galactica.nrepl.client.impl.async/close-topic recv|p topic))
                         error| (chan 1)
...
#_(defn nrepl
  ...
  (let [proc| (chan 10)
        ...
        nrepl|p (galactica.nrepl.client.impl.async/pub nrepl| topic-fn (fn [_] (sliding-buffer 10)))
        ...
        (-nrepl-op [_ opts]
          (let [{:keys [done-keys op-data result-keys]
                ...
                release #(do
                           ...
                           (galactica.nrepl.client.impl.async/close-topic nrepl|p topic))
                ex| (chan 1)
